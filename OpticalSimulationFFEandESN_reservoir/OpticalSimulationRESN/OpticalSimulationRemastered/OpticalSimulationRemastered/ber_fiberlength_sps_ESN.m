...
fiber_lengths = [80 ] * 1e3; % Fiber lengths in meters
sample_per_symbols = [2] ;
...
for x_index = 1:length(sample_per_symbols)
    ...
    for y_index = 1:length(fiber_lengths)
        ...
        [ber_esn] = ESN(points_per_symbol,M,y,n_ignore,adc_rate,symbol_rates,number_of_symbols,photocurrent);
        
        % Store BER value
        ber_matrix(x_index, y_index) = ber_esn;
    end
end

disp(log10(ber_matrix));
