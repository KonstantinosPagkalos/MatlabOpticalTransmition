...
fiber_lengths = [20 30 40 50 60 65 70 75] * 1e3; % Fiber lengths in meters
sample_per_symbol = 1;
...
for x_index = 1:length(tapVector)
    ...
    for y_index = 1:length(fiber_lengths)
        ...
        disp(['tap = ', num2str(taps), ', Fiber Length = ', num2str(fiber_length / 1e3), ' km']);
        ...
        [number_of_symbols, n_ignore, ~, ~, X_for_network, y_for_network, ~] = post_processing(taps, photocurrent, y, points_per_symbol, symbol_rates, adc_rate, number_of_symbols, n_ignore);
        
        % Ridge regression for the optical dataset
        ber = RidgeReg(M, X_for_network, y_for_network);
        disp(['Ber from ridge= ', num2str(ber)]);
