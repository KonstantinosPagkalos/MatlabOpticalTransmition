...
ChallengePuf(:,1) = ones(sequenceLength,1);
ChallengePuf(:,2) = rand(sequenceLength,1)>0.5;
...
%%%% generate an esn 
nInputUnits = 2; nInternalUnits = 30; nOutputUnits = 10; 
% 
teachscale=ones(nOutputUnits,1).*0.3;
teachshift=ones(nOutputUnits,1).*(-0.2);

esn = generate_esn(nInputUnits, nInternalUnits, nOutputUnits, ...
    'spectralRadius',0.5,'inputScaling',[0.1;0.1],'inputShift',[0;0], ...
    'teacherScaling',teachscale,'teacherShift',teachshift,'feedbackScaling', 0, ...
    'type', 'plain_esn'); 



nForgetPoints = 1 ; 
predictedTrainOutput = test_esn(ChallengePuf, esn, nForgetPoints);
